...
- Кнопка режима preview на карточке (в левом верхнем углу): цикл `A -> R -> C`
...
- `Resolution`: размер холста для рендера берётся из размера окна доски (квантование до 32px)
