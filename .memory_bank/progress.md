...
- В drag-out payload убрана дедупликация карточек:
  - при перетаскивании нескольких карточек одинакового prefab они не фильтруются по уникальности на этапе сборки payload.
- Добавлен resize групп через handles (края + углы), изменение размера undoable.
- Добавлена правая панель `Board Items`:
  - выводит отдельные секции `Anchors` и `Elements`;
  - клик по item фокусирует карточку;
  - клик по group фреймит и центрирует группу.
- Добавлены горячие клавиши:
  - `Ctrl/Cmd + Z` undo
  - `Ctrl/Cmd + Y` / `Ctrl/Cmd + Shift + Z` redo
  - Canvas синхронизирует UI через `Undo.undoRedoPerformed`.
- Переработан старт drag/resize группы с использованием координат `evt.currentTarget`, чтобы устранить кейс «группа не таскается».
- Для drag/resize групп добавлен canvas-level hit-test (`TryHitGroup`), чтобы interaction не зависел от доставки pointer событий в дочерний элемент.
- Группы в текущем UX работают как anchors:
  - `Add To Group` убран из контекстного меню;
  - drag anchor не двигает карточки;
  - создание anchor не присваивает `item.groupId`.
- Для undo/redo убраны записи camera-only операций (`zoom/reset/frame/focus`), чтобы `Ctrl+Z/Y` откатывали изменения элементов/групп.
- Multi-board storage переведён на отдельные board asset-файлы:
...
- Нужна ручная проверка Unity drop-пайплайна на предмет того, как `Scene/Hierarchy` обрабатывает повторяющиеся `objectReferences` одного prefab.
...
- Для group resize нужна ручная UX-проверка (попадание в handles, удобство на разных zoom).
- Для optional-режима «приклеивания к группе» пока нет финализированных правил (верхняя группа + запрет пересечений рассматриваются как следующий этап).
- `BoardLibraryAsset` остаётся в проекте как legacy-артефакт, но больше не является источником списка досок.
...
- Проверка изменений после `last_checked_commit` выполнена:
  - `git log 0874859...HEAD`
  - найдено: `7794178`, `b8db117`.

## Контроль изменений
- last_checked_commit: b8db117
